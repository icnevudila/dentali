-- Fix: get_effective_settings is STABLE but was INSERTing organization_settings
-- and calling ensure_branch_clinic_hours (writes) → same read-only transaction class of bugs.
-- Seed hours via ensure_branch_clinic_hours from the client / bootstrap instead.
...
-- Backfill missing org settings / clinic hours so read path stays complete.
...
begin
  for r in select id from public.branches loop
    perform public.ensure_branch_clinic_hours(r.id);
  end loop;
end $$;
