-- Collections AR worklist: keep issued open AR separate from draft
-- invoices with balance (not yet issued — honest "drafts" bucket).
...
comment on function public.list_collections_ar_worklist(uuid, int) is
  'Lists issued open AR (sent/partial) plus separate draft-with-balance rows; billing.read; Asia/Manila aging; no phone.';
