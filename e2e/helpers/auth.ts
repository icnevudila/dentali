...
export function requireE2eCredentials() {
  if (process.env.E2E_STRICT === "true" && (!e2eEmail || !e2ePassword)) {
    throw new Error("E2E_TEST_EMAIL and E2E_TEST_PASSWORD are required in strict staging mode")
  }
  test.skip(
...
export async function loginAsTestUser(page: Page) {
  ...
  if (page.url().includes("/onboarding")) {
    if (process.env.E2E_STRICT === "true") {
      throw new Error("The staging E2E account must be fully onboarded")
    }
