...
export function NotificationChannelSettingsPanel({ branchId, canWrite }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    setMessage(null)
    const { data, error: err } = await upsertNotificationChannelSettings({
      ...settings,
      branch_id: branchId,
    })
    setSaving(false)
    ...
    setMessage(
      data?.dry_run
        ? t(
            ...
            "Dry-run: email was not sent. Disable email dry-run and add RESEND_API_KEY to go live."
          )
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-3">
          <ProviderCard
            title="SMS"
            icon={MessageSquare}
            configured={health?.sms.configured ?? false}
            provider={health?.sms.provider ?? "Semaphore"}
            secretName={health?.sms.secret_name ?? "SEMAPHORE_API_KEY"}
            hint={t(
              "settings.channelSmsSecretHint",
              "Add API key in Supabase → Edge Functions → Secrets. Sender name below overrides the default."
            )}
          />
          <ProviderCard
            title={t("settings.channelEmailTitle", "Email")}
            icon={Mail}
            configured={health?.email.configured ?? false}
            provider={health?.email.provider ?? "Resend"}
            secretName={health?.email.secret_name ?? "RESEND_API_KEY"}
            hint={t(
              "settings.channelEmailSecretHint",
              "Free tier: ~100 emails/day. Verify your domain in Resend, then set the from address below."
            )}
          />
          <ProviderCard
            title="WhatsApp"
            ...
            secretName={t("settings.channelNoKey", "No API key")}
            hint={
          ...
          <CardContent className="space-y-4">
            <ToggleRow
              ...
              description={t(
                ...
                "When on, emails are preview-only until you add RESEND_API_KEY and turn this off."
              )}
...
function ProviderCard({
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-white p-4">
      ...
      <p className="mt-1 font-mono text-[11px] text-neutral-400">{secretName}</p>
