...
export function NotificationAnalyticsPanel({ branchId }: { branchId: string }) {
  const { t } = useLocale()
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data } = await fetchNotificationAnalytics(branchId, 30)
    if (data) {
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-neutral-200/80 bg-white px-4 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
          <p className="text-xs font-medium text-neutral-500">
            {t("notifications.deliveryRate", "Delivery rate (30d)")}
          </p>
        ...
        <div className="rounded-xl border border-neutral-200/80 bg-white px-4 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
          <p className="text-xs font-medium text-neutral-500">
            {t("notifications.totalSent", "Messages sent (30d)")}
          </p>
          <p className="mt-1 text-2xl font-semibold tabular-nums text-neutral-900">
            {loading ? "—" : totalSent}
          </p>
