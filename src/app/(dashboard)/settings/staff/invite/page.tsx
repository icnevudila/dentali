...
export default function StaffInvitePage() {
  ...
  // Invite-first default (email link); direct add remains available for fast setup.
  ...
  const roleLabel = (name: string) => {
    ...
    return t(key, fallbacks[name] ?? name)
  }
  ...
  return (
    <PermissionGate permission={PERMISSIONS.STAFF_MANAGE}>
      <ModulePageShell
        icon={UserPlus}
        eyebrow={t("settings.staffEyebrow", "Settings · Staff")}
        title={
          addMode === "direct"
            ? t("settings.addStaffDirectTitle", "Add Staff Member Directly")
            : t("settings.inviteStaffTitle", "Invite Staff Member")
        }
        description={
          addMode === "direct"
            ? t(
                "settings.addStaffDirectDesc",
                "Directly register a provider or doctor to the branch database."
              )
            : t(
                "settings.inviteStaffDesc",
                "Send an email invitation link to join your organization."
              )
        }
        maxWidth=""
        ...
        actions={
          <Button variant="ghost" size="icon" asChild>
            <Link href="/settings/staff" aria-label={t("settings.backToStaff", "Back to staff")}>
        ...
        <ContentPanel>
          <div className="flex gap-2 mb-4 p-1 bg-neutral-100 rounded-lg w-fit">
            ...
            >
              ...
              {t("settings.sendInviteEmail", "Send Invite Email")}
            </button>
            <button
              type="button"
            ...
            >
              ...
              {t("settings.addDirectlyFast", "Add Directly (Fast Setup)")}
            </button>
          </div>
          ...
          <Card className="border-0 shadow-none">
            <CardHeader className="px-0 pt-0">
              <CardTitle className="text-base">
                {addMode === "direct"
                  ? t("settings.staffInformation", "Staff Information")
                  : t("settings.invitationDetails", "Invitation Details")}
              </CardTitle>
              <CardDescription>
                {addMode === "direct"
                  ? t(
                      "settings.addStaffDirectHint",
                      "Bypass invitation emails and add the profile directly to testing/production branch."
                    )
                  : t(
                      "settings.inviteStaffHint",
            ...
            <CardContent className="px-0 pb-0">
              <form onSubmit={handleInvite} className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-xs font-medium">{t("common.email", "Email")}</label>
                  <Input
                    type="email"
                    required
                    value={inviteEmail}
                    onChange={(e) => setInviteEmail(e.target.value)}
                    placeholder={t("settings.staffEmailPlaceholder", "staff@clinic.com")}
                  />
                </div>
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-xs font-medium">{t("settings.fullName", "Full Name")}</label>
                  <Input
                    required
                    value={inviteName}
                    onChange={(e) => setInviteName(e.target.value)}
                    placeholder={t("settings.staffNamePlaceholder", "Dr. Maria Santos")}
                  />
                </div>
                ...
                {addMode === "direct" ? (
                  <>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium">
                        {t(
                          "settings.specializationLabel",
                          "Specialization (e.g., Ortho, Pedo)"
                        )}
                      </label>
                      <Input
                        ...
                        placeholder={t(
                          "settings.specializationPlaceholder",
                          "General Dentistry"
                        )}
                      />
                    </div>
                    <div className="space-y-1 sm:col-span-2">
                      <label className="text-xs font-medium">
                        {t("settings.prcLicenseNumber", "PRC license number")}
                      </label>
                      <Input
                        value={prcLicenseNumber}
                        onChange={(e) => setPrcLicenseNumber(e.target.value)}
                        placeholder={t("settings.prcLicensePlaceholder", "e.g. PRC 1234567")}
                      />
                      <p className="text-xs text-neutral-500">
                        {t(
                          "settings.prcLicenseHint",
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium">{t("settings.branch", "Branch")}</label>
                  <select
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium">{t("settings.role", "Role")}</label>
                  <select
                    value={inviteRoleId}
                  ...
                  <Button type="submit" disabled={inviting}>
                    {inviting
                      ? addMode === "direct"
                        ...
                        : t("settings.sendingInvite", "Sending…")
                      : addMode === "direct"
                        ? t("settings.addStaffMember", "Add Staff Member")
                        : t("settings.sendInviteEmail", "Send Invite Email")}
                  </Button>
                  <Button type="button" variant="outline" asChild>
                    <Link href="/settings/staff">{t("common.cancel", "Cancel")}</Link>
                  </Button>
                </div>
              </form>
            </CardContent>
