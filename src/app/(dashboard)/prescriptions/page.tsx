...
export default function GlobalPrescriptionsPage() {
  ...
  const metrics: MetricItem[] = [
    {
      label: "Total Issued e-Rx",
      value: String(RECENT_CLINIC_PRESCRIPTIONS.length),
      hint: "Clinic-wide prescriptions",
    },
    {
      label: "Signed & Verified",
      value: String(RECENT_CLINIC_PRESCRIPTIONS.filter((r) => r.status === "signed").length),
      hint: "Signed by licensed dentist",
    },
    {
    ...
    {
      label: "Draft Pending",
      value: String(RECENT_CLINIC_PRESCRIPTIONS.filter((r) => r.status === "draft").length),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hallmark Standard Page Header */}
      <PageHeader
        eyebrow={<SectionEyebrow icon={Pill}>CLINICAL · e-PRESCRIPTIONS</SectionEyebrow>}
        title="e-Prescriptions & Clinical Rx Protocols"
        description="Manage dental prescriptions, protocol presets, and patient medication records across the clinic."
        actions={
          <Button
      ...
      {/* Preset Clinical Rx Protocol Packs */}
      <ContentPanel>
        <div className="mb-4 space-y-0.5">
          ...
          <p className="text-xs text-slate-500">
            1-click protocol templates for acute pain, surgical post-op, and periodontal infections
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {DENTAL_PRESCRIPTION_PRESETS.map((preset) => (
            <Card key={preset.name} className="border-slate-200 bg-white hover:border-teal-300 transition-colors shadow-xs">
      ...
      {/* Issued Clinic Prescriptions History Table */}
      <ContentPanel>
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-slate-500">
              Searchable archive of patient e-prescriptions and medication printouts
            </p>
          </div>
          ...
          <div className="relative w-full sm:w-80">
            ...
            <Input
              type="text"
              placeholder="Filter by patient name, Rx protocol or dentist..."
              value={searchQuery}
                    ...
                    <tr key={rx.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-3">
                        {rx.status === "signed" ? (
                          ...
                          <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 text-[10px] gap-1 font-semibold">
                      ...
                      <td className="p-3 text-right">
                        <Button asChild size="sm" variant="ghost" className="h-8 text-xs gap-1 text-teal-700 hover:text-teal-800 hover:bg-teal-50 font-semibold">
